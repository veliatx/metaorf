...
if __name__ == "__main__":
    ...
    if "multimap" not in st.session_state:
        st.session_state['multimap'] = 10
    if "orfcallers" not in st.session_state:
        st.session_state['orfcallers'] = []
    ...
    with tab1:
        ...
        st.session_state["orfcallers"] = st.multiselect(
            label='ORF callers to run', options=['price', 'ribocode', 'ribotish'])
        st.session_state["multimap"] = st.number_input("Max number of multiple alignments allowed for a read", value=10)
