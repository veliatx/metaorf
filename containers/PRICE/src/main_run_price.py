"""
This python program processes Ribo-seq data and uses Price to identify ORFs.
"""
...
def is_validated(qc_file, log_file):
    """Filter requirement:
	    ...
	    b. ≥ 2 million usable reads (multimappers + uniquely mapped reads)
...
def main(annotation_dir, output_dir, experiment_name, reference_genomes, genome_annotation_prefix):
    ...
    price_folder = init_folder(Path(f'{output_dir}/price_results/'))
    if is_validated(qc_file=f'{aligned_reads_prefix}_qc.tsv', log_file=f'{aligned_reads_prefix}_Log.final.out'):
        # prepare the index for full DNA reference genomes.
        oml_file = Path(f'{price_folder}/{genome_annotation_prefix}.oml')
        price_functions.generate_index_for_genome(
                fasta_file=" ".join(reference_fasta),
                gtf_file=gtf_file,
                output_prefix=oml_file)
        
        # call Price for ORF calling.
        reads_file = Path(f'{aligned_reads_prefix}_Aligned.filtered.sorted.bam')
        price_result_prefix = Path(f'{price_folder}/{experiment_name}')
        price_functions.call_ribo_seq_peaks(
            reads_file=reads_file,
            genomic_meta_file=oml_file,
            price_result=price_result_prefix)
        price_functions.view_cit(prefix=price_result_prefix)
    else:
        print("Price did not run.")
    print("Price has completed the task.")
