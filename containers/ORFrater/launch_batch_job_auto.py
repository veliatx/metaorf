...
def submit_list_job(experiment_name, parameter_filepath, dependencies):
    ...
    response = boto3.client('batch', 'us-west-2').submit_job(
        ...
        jobDefinition='arn:aws:batch:us-west-2:328315166908:job-definition/orfrater:2',
        containerOverrides = {
            'command': ['ls', '-R', '/'],
        },
...
def submit_orf_caller_job(experiment_name, parameter_filepath, dependencies):
    ...
    orfrater_cmd = [
        'python3', 'src/main_run_orfrater.py',
        ...
        '--gene_names', parameters['gene_names'],
        '--pseudogenes', parameters['pseudogenes'],
        '--transcriptome_bed_file', parameters['transcriptome_bed_file']]
    
    # CHX
    if riboseq_filenames[0].endswith("fastq.gz") or riboseq_filenames[0].endswith("fastq"):
        orfrater_cmd += ['--samples', ','.join(riboseq_filenames)]
        if 'umi' in parameters and parameters['umi']:
            orfrater_cmd += ['--umi_pattern', parameters['umi']]
        if 'adapter_sequence' in parameters and parameters['adapter_sequence']:
            orfrater_cmd += ['--adapter_sequence', parameters['adapter_sequence']]
    elif riboseq_filenames[0].endswith("bam"):
        orfrater_cmd += ['--sample_bam_files', ','.join(riboseq_filenames)]
    else:
    ...
    if 'tis_paths' in parameters:
        ...
        if tis_filenames[0].endswith("fastq.gz") or tis_filenames[0].endswith("fastq"):
            orfrater_cmd += ['--fastq_tis_files', ','.join(tis_filenames)]
            if 'umi_pattern_tis' in parameters and parameters['umi_pattern_tis']:
                orfrater_cmd += ['--umi_pattern_tis', parameters['umi_pattern_tis']]
            if 'adapter_sequence_tis' in parameters and parameters['adapter_sequence_tis']:
                orfrater_cmd += ['--adapter_sequence_tis', parameters['adapter_sequence_tis']]
        elif tis_filenames[0].endswith("bam"):
            orfrater_cmd += ['--bam_tis_files', ','.join(tis_filenames)]
        else:
            exit(f'Unexpected type of input {tis_filenames[0]}')

    if 'skip_orfcalling' in parameters and parameters['skip_orfcalling'] != "False":
        orfrater_cmd += ['--skip_orfcalling']

    response = boto3.client('batch', 'us-west-2').submit_job(
        jobName=f'{experiment_name}_orfrater',
        jobQueue='bfx-jq-general',
        jobDefinition='arn:aws:batch:us-west-2:328315166908:job-definition/orfrater:2',
        containerOverrides = {
            'command': orfrater_cmd,
        },
        dependsOn=[{'jobId': job_id, 'type': 'N_TO_N'} for job_id in dependencies],
    )
