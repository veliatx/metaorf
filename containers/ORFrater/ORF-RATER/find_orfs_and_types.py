...
with open('%s.txt' % opts.tfamstem) as tfamtable:
    for line in tfamtable:
        ls = line.strip().split()
        tfamtids[ls[1]].append(ls[0])

with open('%s.bed' % opts.tfamstem) as tfambed:
...
if opts.extracdsbeds:
    ...
    for cdsbedfile in opts.extracdsbeds:
        with open(cdsbedfile) as cdsbed:
...
tfams_with_annots = set(sum([list(x.keys()) for x in annot_tfam_lookups], []))
...
def _identify_tfam_orfs(tup):
    ...
    tmask = np.empty((len(tids), len(tfam_genpos)), dtype=bool)  # True if transcript covers that position, False if not
    tfam_orfs = []
    tidx_lookup = {}
    for tidx, tid in enumerate(tids):
        ...
        if trans_orfs:
            (startpos, stoppos, codons) = list(zip(*trans_orfs))
            ...
            tfam_orfs.append(pd.DataFrame.from_dict(OrderedDict([
                                                      ('tfam', tfam),
                                                      ('tid', tid),
                                                      ('tcoord', startpos),
                                                      ('tstop', stoppos),
        ...
        for ((gcoord, AAlen), gcoord_grp) in tfam_orfs.groupby(['gcoord', 'AAlen']):  # group by genomic start position and length
            if len(gcoord_grp) == 1:
                ...
                if (orf_gcoords == orf_gcoords[0, :]).all():  # all of the grouped ORFs are identical, so should receive the same name
                    ...
                    named_so_far = 0
                    unnamed = np.ones(len(gcoord_grp), dtype=bool)
        ...
        if tfam in tfams_with_annots:
            ...
            if cds_info:  # False means no annotated CDSs or none are multiples of 3 in length
                cds_info = pd.DataFrame(cds_info, columns=['gcoord', 'gstop', 'AAlen', 'in_tfam', 'annot_fidx', 'annot_tid', 'pos']) \
                    ...
                    .apply(lambda x: x if len(x) == 1 else x[[not any(pos == x['pos'].iat[j] for j in range(i))
...
all_orfs = pd.concat(workers.map(_identify_tfam_orfs, tfamtids.items()), ignore_index=True)
workers.close()
