...
def ReadKeyMapFactory(offset_dict, read_key_fun=lambda read: len(read.positions)):
    ...
    map_func.read_keys = list(offset_dict.keys())
...
class HashedReadBAMGenomeArray(BAMGenomeArray):
    ...
    def get_reads_and_hashed_counts(self,roi,roi_order=True):
        ...
        # filter by strand
        if roi.strand == "+":
            reads = filter(lambda x: not x.is_reverse, reads)
        elif roi.strand == "-":
            reads = filter(lambda x: x.is_reverse, reads)
        ...
        if self._normalize:
            count_dict = {k: v*1.0e6/self.sum() for (k, v) in count_dict.items()}
    ...
    def get_reads_and_counts(self,roi,roi_order=True):
        ...
        return reads, sum(list(count_dict.values()), numpy.zeros(len(roi)))
