"""
This python program processes Ribo-seq data and uses ORF-RATER to identify ORFs.
The details of ORF-RATER can be found on https://github.com/alexfields/ORF-RATER

Args:
    --sample_id: name of the sample to process. The reads file of `sample_id` should be stored in the `input_dir` as `sample_id.fastq.gz`. ORFs identified by Price will be saved into the `output_dir` with the filename prefix as `sample_id`.
    
    --input_dir: path to the input directory. The `input_dir` needs to contain the following contents and hierarchy:
        - input_dir
            - sample_{1,2,3,...}.fastq.gz or sample_{1,2,3,...}.bam
            - annotation
                - reference_genome_{1,2,3,...}.fa
                - contaminant_genome_{1,2,3,...}.fa
                - star_contaminant_riboseq_index
                - star_hg38_riboseq_index
                
    --output_dir: path to the input directory. Orfs identified by Price can be found in the `price_results` folder in `output_dir`.

Usage example:
    python3 main_run_price.py \
            --experiment_name SRR9113067_1 \
            --samples SRR9113067_1.fastq.gz \
...
def map_reads_to_reference(reads_file, contaminant_index, contaminants_prefix,
    ...
    star_functions.map_reads_to_reference(
        ...
        out_filter_mismatch_Nmax=2,
        transcriptome_SAM=True,
        random_multimapper_order=True)
...
def prepare_bam_files(aligned_reads_prefix, umi_pattern):
    ...
    samtools_functions.sort_bam_file(
        unsorted_bam_file=Path(f'{aligned_reads_prefix}_Aligned.out.bam'),
        sorted_bam_file=Path(f'{aligned_reads_prefix}_Aligned.sorted.bam'))

    if umi_pattern:
        samtools_functions.remove_duplicates(
            raw_bam_file=Path(f'{aligned_reads_prefix}_Aligned.sorted.bam'),
            filtered_bam_file=Path(f'{aligned_reads_prefix}_Aligned.filtered.tmp.sorted.bam'))
        samtools_functions.index_reads(
            bam_file=Path(f'{aligned_reads_prefix}_Aligned.filtered.tmp.sorted.bam'))
        umitools_functions.dedup(
                input_bam=Path(f'{aligned_reads_prefix}_Aligned.filtered.tmp.sorted.bam'),
                output_bam=Path(f'{aligned_reads_prefix}_Aligned.filtered.sorted.bam'),
                stats_prefix=Path(f'{aligned_reads_prefix}_umi_stats'))
    else:
        samtools_functions.remove_duplicates(
            raw_bam_file=Path(f'{aligned_reads_prefix}_Aligned.sorted.bam'),
            filtered_bam_file=Path(f'{aligned_reads_prefix}_Aligned.filtered.sorted.bam'))
...
def create_bam_file(samples, sample_bam_files, annotation_dir, input_dir, output_dir, suffix, experiment_name,
    ...
    if not sample_bam_files:
        ...
        map_reads_to_reference(
            reads_file=Path(f'{processed_reads_prefix}.trimmed.fastq'),
...
def create_log_for_dup(output_log, reads_prefix, umi_pattern):
    ...
    with open(output_log, 'w') as output_file:
        n_reads_no_filtering = compute_read_number_in_bam(Path(f'{reads_prefix}_Aligned.sorted.bam'))
        output_file.write(
                f'Number of reads before filtering\t'
                f'|\t{n_reads_no_filtering}\n')
        
        if umi_pattern:
            n_reads_no_secondary = compute_read_number_in_bam(Path(f'{reads_prefix}_Aligned.filtered.tmp.sorted.bam'))
            output_file.write(
                    f'Number of reads after removing secondary alignments\t'
                    f'|\t{n_reads_no_secondary}\n')
            output_file.write(
                    f'% reads are removed due to secondary alignments\t'
                    f'|\t{1 - n_reads_no_secondary/float(n_reads_no_filtering)}\n')
            ...
            output_file.write(
                    ...
                    f'|\t{1 - n_reads_after_umi/float(n_reads_no_secondary)}\n')
        else:
            n_reads_no_secondary = compute_read_number_in_bam(Path(f'{reads_prefix}_Aligned.filtered.sorted.bam'))
            output_file.write(
                    f'Number of reads after removing secondary alignments\t'
                    f'|\t{n_reads_no_secondary}\n')
            output_file.write(
...
def main(annotation_dir, input_dir, output_dir, samples, sample_bam_files,
         ...
         orfrater_source_folder, qc_source_folder, fastq_tis_files, bam_tis_files,
         min_read_length, max_read_length, transcriptome_bed_file,
         gene_names, pseudogenes, exclude_genes, init_codon, skip_orfcalling,
         multimap_Nmax=10):
    ...
    if exclude_genes == 'auto':
        ...
        exclude_genes = exclude_genes.strip().split(",")

    if (not skip_orfcalling
        and is_validated(qc_file=f'{aligned_reads_prefix}_qc.tsv', log_file=f'{aligned_reads_prefix}_Log.final.out')):
        orfrater_functions.run_orfrater_pipeline(
                orfrater_source_folder=orfrater_source_folder,
                bam_files=[Path(f'{aligned_reads_prefix}_Aligned.filtered.sorted.bam')] ,
                bam_tis_file=tis_bam_merged,
                orfrater_data_folder=Path(f'{output_dir}/orfrater_results'),
                min_read_length=min_read_length,
                max_read_length=max_read_length,
                genome_fasta_file=" ".join(reference_fasta),
                pseudogenes=Path(f'{annotation_dir}/gene_names/{pseudogenes}'),
                transcriptome_bed_file=Path(f'{annotation_dir}/annotations/{transcriptome_bed_file}'),
                gene_names=Path(f'{annotation_dir}/gene_names/{gene_names}'),
                exclude_genes=exclude_genes,
                init_codon=init_codon)
    ...
    print("Orf-rater has completed the task.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--adapter_sequence_tis', type=str, default='auto',
                        help=('Adapter sequence for TIS reads. If not specified, '
                              'the adapter will be auto-detected. Default: %(default)s'))
    parser.add_argument('--orfrater_source_folder', type=str, default='ORF-RATER',
                        help=('Path to the folder where the ORF-rater source code is. '
                              'Default: %(default)s'))
    ...
    parser.add_argument('--fastq_tis_files', type=str, default=None,
                        help=('Path to the transcriptome-aligned TIS fastq file. Set this as None '
                              'if no TIS file is used. Default: %(default)s'))
    parser.add_argument('--min_read_length', type=int, default=24,
                        help=('Min length of footprint to consider. Default: %(default)s'))
    parser.add_argument('--max_read_length', type=int, default=34,
                        help=('Max length of footprint to consider. Default: %(default)s'))
    parser.add_argument('--gene_names', type=str,
                        help=('Path to the list of gene names.'))
    parser.add_argument('--pseudogenes', type=str,
                        help=('Path to the list of pseudogenes.'))
    parser.add_argument('--transcriptome_bed_file', type=str,
                        help=('Path to the bed file of transcriptomes.'))
    parser.add_argument('--exclude_genes', type=str, default='auto',
                        help=('List of genes to exclude from ORF-rater calling. Set as auto when using '
                              'the default gene list. Default: %(default)s'))
    parser.add_argument('--init_codon', type=str, default='NTG',
                        help=('Codons to consider as possible translation initiation sites.'
                              'Seperated by comma. Default: %(default)s'))
    parser.add_argument('--skip_orfcalling', action='store_true', help=('Specify this flag to skip orf-calling.'))
    ...
    main(annotation_dir=Path(args.annotation_dir),
         ...
         orfrater_source_folder=Path(args.orfrater_source_folder),
         ...
         transcriptome_bed_file=args.transcriptome_bed_file,
         gene_names=args.gene_names,
         pseudogenes=args.pseudogenes,
         exclude_genes=args.exclude_genes,
         init_codon=args.init_codon,
         skip_orfcalling=args.skip_orfcalling)
