"""
This python program processes Ribo-seq data and uses RibORF.2.0 to identify ORFs.
"""
...
def main(annotation_dir, output_dir, experiment_name, reference_genomes, genome_annotation_prefix):
    ...
    init_folder(Path(f'{output_dir}/riborf_results/'))
    if True: #is_validated(qc_file=f'{aligned_reads_prefix}_qc.tsv', log_file=f'{aligned_reads_prefix}_Log.final.out'):
        riborf_functions.run_riborf_pipeline(
                transcriptome_bam_file=Path(f'{aligned_reads_prefix}_Aligned.toTranscriptome.out.sam'),
                offset_file=Path(f'{aligned_reads_prefix}_qc.tsv'),
                genome_annotation_prefix=Path(f'{annotation_dir}/annotations/{genome_annotation_prefix}'),
                riborf_data_folder=Path(f'{output_dir}/riborf_results/'),
                genome_fasta_file=" ".join(reference_fasta))
    else:
        print("RibORF did not run.")
    print("RibORF has completed the task.")
