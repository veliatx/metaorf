"""
This python program processes Ribo-seq data and uses RiboTISH to identify ORFs.
"""
...
def main(annotation_dir, output_dir, experiment_name, reference_genomes, genome_annotation_prefix):
    ...
    if os.path.exists(f'{output_dir}/aligned_tis'):
        aligned_tis_reads = Path(f'{output_dir}/aligned_tis/{experiment_name}_Aligned.filtered.sorted.bam')
    else:
        aligned_tis_reads = None

    init_folder(Path(f'{output_dir}/ribotish_results/'))
    if is_validated(qc_file=f'{aligned_reads_prefix}_qc.tsv', log_file=f'{aligned_reads_prefix}_Log.final.out'):
        ribotish_functions.run_ribotish_pipeline(
                bam_file= Path(f'{aligned_reads_prefix}_Aligned.unique.sorted.bam'),
                bam_tis_file = aligned_tis_reads,
                annotation_gtf_file=gtf_file,
                ribotish_data_folder=Path(f'{output_dir}/ribotish_results'),
                genome_fasta_file=" ".join(reference_fasta))
    else:
        print("RiboTISH did not run.")
    print("RiboTISH has completed the task.")
