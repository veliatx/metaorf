"""
This python program processes Ribo-seq data and uses RiboCode to identify ORFs.
"""
...
def main(annotation_dir, output_dir, experiment_name, reference_genomes, genome_annotation_prefix):
    ...
    init_folder(Path(f'{output_dir}/ribocode_results/'))
    if is_validated(qc_file=f'{aligned_reads_prefix}_qc.tsv', log_file=f'{aligned_reads_prefix}_Log.final.out'):
        ribocode_functions.run_ribocode_pipeline(
                transcriptome_bam_file=Path(f'{aligned_reads_prefix}_Aligned.toTranscriptome.out.bam'),
                annotation_gtf_file=gtf_file,
                ribocode_data_folder=Path(f'{output_dir}/ribocode_results/ribocode_results'),
                genome_fasta_file=" ".join(reference_fasta))
    else:
        print("RiboCode did not run.")
    print("RiboCode has completed the task.")
